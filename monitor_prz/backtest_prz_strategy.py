"""
10年期 PRZ 諧波交易策略遞迴歷史回測與驗證系統 (Backtesting Engine v2.0)
=============================================================
1. 抓取近 10 年 (2016 - 2026) 台指期 / 加權日 K 線數據 (2,428 筆交易日)
2. 滾動窗口 (Rolling Window) 遞迴執行 PRZ 演算法與交易建議
3. 支援【全訊號基線測試】與【高信心水準共振濾網測試】對比
...
def run_backtest(df, tracker, mode=3, high_confidence_only=True, max_holding_days=10):
    """
    執行 10 年期滾動式遞迴回測
    high_confidence_only: True 表示僅於「高信心水準 (超級共振)」時才進場
    """
    trades = []
    total_bars = len(df)
    START_INDEX = 60
    
    filter_label = "【超級共振高信心水準濾網】" if high_confidence_only else "【全訊號基線測試】"
    print(f"\n⏳ 開始執行滾動式遞迴回測 ({filter_label}, 共 {total_bars - START_INDEX} 個交易日)...")
    ...
    while i < total_bars - max_holding_days:
        try:
            ...
            if entry_dir in ['做多', '做空']:
                ...
                # 依日 K 波動屬性調整結構停損位與停利位
                if len(stop_losses) >= 2:
                    sl_price = stop_losses[1]['price']  # 採用二階標準停損，防範日線極限洗盤
                else:
                    sl_price = stop_losses[0]['price']
...
def evaluate_performance(trades, label="測試報告"):
    ...
    print(f"  🎯 停利達標率(TP1): {(len(df_trades[df_trades['result']=='TP1'])/total_trades)*100:.1f}%")
    print("=" * 65)
...
def main():
    ...
    print("  🛡️ 內建 3 次失敗中斷保護機制 (3-Strike Rule Guard)")
    print("=" * 65)
    ...
    # 測試 1: 全訊號基線測試
    trades_base = run_backtest(df_10y, tracker, mode=3, high_confidence_only=False, max_holding_days=10)
    evaluate_performance(trades_base, label="全訊號基線測試")
    
    # 測試 2: 高信心水準共振濾網測試 (建議發信觸發條件)
    trades_high_conf = run_backtest(df_10y, tracker, mode=3, high_confidence_only=True, max_holding_days=10)
    evaluate_performance(trades_high_conf, label="🔥高信心水準超級共振濾網測試 (即時監控發信門檻)")
